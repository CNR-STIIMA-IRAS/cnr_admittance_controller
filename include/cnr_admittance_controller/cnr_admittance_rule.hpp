...
#ifndef CNR_ADMITTANCE_CONTROLLER__ADMITTANCE_RULE_HPP_
#define CNR_ADMITTANCE_CONTROLLER__ADMITTANCE_RULE_HPP_
...
#include "cnr_admittance_controller/cnr_admittance_controller_parameters.hpp"
...
public:
  explicit AdmittanceRule(
    const std::shared_ptr<cnr_admittance_controller::ParamListener> & parameter_handler)
  {
  ...
  controller_interface::return_type configure(
    ...
    const std::string & robot_description, const double& control_frequency, const double& force_deadband,
    const double& torque_deadband, const int64_t& bias_cycles);
...
public:
  ...
  std::shared_ptr<cnr_admittance_controller::ParamListener> parameter_handler_;
  cnr_admittance_controller::Params parameters_;
...
protected:
   ...
   */
  int64_t bias_cycles_;
  int64_t bias_cycles_cnt_;  
  std::shared_ptr<eigen_control_toolbox::FirstOrderLowPassX> force_lpf_; // the same for FirstOrderHighPass
  std::shared_ptr<eigen_control_toolbox::FirstOrderLowPassX> torque_lpf_; // the same for FirstOrderHighPass
  double force_deadband_;
  double torque_deadband_;
  Eigen::VectorXd force_bias_;
  Eigen::VectorXd torque_bias_;
  ...
  AdmittanceState admittance_setpoint_{0};
...
#endif  // CNR_ADMITTANCE_CONTROLLER__ADMITTANCE_RULE_HPP_

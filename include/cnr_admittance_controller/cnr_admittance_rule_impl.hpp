...
// limitations under the License.
//
/// \authors: Nicola Pedrocchi, Adriano Scibilia

#ifndef CNR_ADMITTANCE_CONTROLLER__ADMITTANCE_RULE_IMPL_HPP_
#define CNR_ADMITTANCE_CONTROLLER__ADMITTANCE_RULE_IMPL_HPP_

#include "cnr_admittance_controller/cnr_admittance_rule.hpp"
...
namespace cnr_admittance_controller
{

constexpr auto NUM_CARTESIAN_DOF = 6;  // (3 translation + 3 rotation)

inline
Eigen::VectorXd deadband(const Eigen::VectorXd& w, const double d)
{
  Eigen::VectorXd ret(w.size());
  for(Eigen::Index i=0;i<w.size();i++)
  {
    ret(i) = w(i) > d ? w(i) - d : w(i) < -d ? w(i) + d : 0;
  }
  return ret;
}
...
controller_interface::return_type AdmittanceRule::configure(
  ...
  const std::string & robot_description, const double& control_frequency, const double& force_deadband, const double& torque_deadband, const int64_t& bias_cycles)
{
  bias_cycles_ = bias_cycles;
  ...
  force_lpf_.reset(new eigen_control_toolbox::FirstOrderLowPassX(
      parameters_.ft_sensor.low_pass_filter_frequency,1.0 / control_frequency,3)); // the same for FirstOrderHighPass
  force_deadband_ = force_deadband;

  torque_lpf_.reset(new eigen_control_toolbox::FirstOrderLowPassX(
      parameters_.ft_sensor.low_pass_filter_frequency,1.0 / control_frequency,3)); // the same for FirstOrderHighPass
  torque_deadband_ = torque_deadband;
...
controller_interface::return_type AdmittanceRule::reset(const size_t num_joints)
{
  ...
  admittance_setpoint_= AdmittanceState(num_joints);
  ...
  bias_cycles_cnt_ = 0;
  force_bias_ = Eigen::Vector3d::Zero();
  torque_bias_ = Eigen::Vector3d::Zero();
...
void AdmittanceRule::apply_parameters_update()
{
  ...
  admittance_setpoint_ = admittance_state_;
}
...
bool AdmittanceRule::calculate_admittance_rule(AdmittanceState & admittance_state, double dt)
{
  ...
  X_ddot = X_ddot.cwiseProduct(admittance_state.selected_axes);
  ...
  // Eigen::Matrix<double, Eigen::Dynamic, 6> j_inverse(4,6);
  // Eigen::VectorXd joint_acc_for_log(4);
  // if (!kinematics_->calculate_jacobian_inverse(admittance_state.current_joint_pos,parameters_.control.frame.id,j_inverse)){
  //     std::cout << "INVERSE JACOBIAN FAILED" << std::endl; 
  // }
  // else{
  //   joint_acc_for_log = j_inverse*X_ddot;
  // }
  ...
  // if ((F_base(0)>10)||(F_base(1)>10)||(F_base(2)>20))
  // {
  //   std::cout << "\n\n\n=== ERRORI GIUNTI ===" << std::endl;
  //   std::cout << "\nadmittance_state.j_acc:\n" << admittance_state.joint_acc.transpose() << std::endl;
  //   std::cout << "\njoint_acc_logged:\n" << joint_acc_for_log.transpose() << std::endl;
  // }
  ...
  const Eigen::Matrix<double, 3, 3> & cog_world_rot)
{
  ...
  if(bias_cycles_cnt_==0)
  {
    force_lpf_->setStateFromLastInput(new_wrench_base.block<3, 1>(0, 0));
    torque_lpf_->setStateFromLastInput(new_wrench_base.block<3, 1>(0, 1));
  }
  
  auto _f = force_lpf_->update(new_wrench_base.block<3, 1>(0, 0));
  auto _t = torque_lpf_->update(new_wrench_base.block<3, 1>(0, 1));

  if(bias_cycles_cnt_++ < bias_cycles_)
  {
    force_bias_ = (force_bias_ + _f) / 2.0;
    torque_bias_ = (force_bias_ + _t) / 2.0;
  }
 
  wrench_world_.block<3, 1>(0, 0) = deadband(_f - force_bias_, force_deadband_ );
  wrench_world_.block<3, 1>(3, 0) = deadband(_t - torque_bias_, torque_deadband_ );
}
...
#endif  // CNR_ADMITTANCE_CONTROLLER__ADMITTANCE_RULE_IMPL_HPP_

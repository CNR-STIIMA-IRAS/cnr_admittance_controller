...
#include "cnr_admittance_controller/cnr_admittance_rule_impl.hpp"
...
controller_interface::CallbackReturn AdmittanceController::on_init()
{
  ...
  try
  {
    parameter_handler_ = std::make_shared<cnr_admittance_controller::ParamListener>(get_node());
    admittance_ = std::make_unique<cnr_admittance_controller::AdmittanceRule>(parameter_handler_);
  }
  ...
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  ...
  // configure admittance rule
    ...
    admittance_->configure(get_node(), num_joints_, this->get_robot_description(), 
      get_update_rate(), admittance_->parameters_.ft_sensor.force_deadband, 
      admittance_->parameters_.ft_sensor.torque_deadband, admittance_->parameters_.ft_sensor.bias_cycles ) ==
...
PLUGINLIB_EXPORT_CLASS(
  cnr_admittance_controller::AdmittanceController, controller_interface::ChainableControllerInterface)

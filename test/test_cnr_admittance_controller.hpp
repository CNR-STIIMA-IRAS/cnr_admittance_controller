...
#include "cnr_admittance_controller/cnr_admittance_controller.hpp"
...
#include "test_cnr_asset_6d_robot_description.hpp"
...
class TestableAdmittanceController : public cnr_admittance_controller::AdmittanceController
{
...
public:
  CallbackReturn on_init() override
  {
    ...
    return cnr_admittance_controller::AdmittanceController::on_init();
  }

  CallbackReturn on_configure(const rclcpp_lifecycle::State & previous_state) override
  {
    return cnr_admittance_controller::AdmittanceController::on_configure(previous_state);
  }
  ...
  void SetUp()
  {
    ...
    force_command_publisher_ =
      command_publisher_node_->create_publisher<ControllerCommandWrenchMsg>(
        "/test_cnr_admittance_controller/force_references", rclcpp::SystemDefaultsQoS());
    // pose_command_publisher_ =command_publisher_node_->create_publisher<ControllerCommandPoseMsg>(
    //    "/test_cnr_admittance_controller/pose_commands", rclcpp::SystemDefaultsQoS());
    joint_command_publisher_ = command_publisher_node_->create_publisher<ControllerCommandJointMsg>(
      "/test_cnr_admittance_controller/joint_references", rclcpp::SystemDefaultsQoS());
...
protected:
  ...
  controller_interface::return_type SetUpController(
    const std::string & controller_name = "test_cnr_admittance_controller")
  {
    auto options = rclcpp::NodeOptions()
                     .allow_undeclared_parameters(false)
                     .automatically_declare_parameters_from_overrides(false);
  ...
  void subscribe_and_get_messages(ControllerStateMsg & msg)
  {
    ...
    auto subscription = test_subscription_node_->create_subscription<ControllerStateMsg>(
      "/test_cnr_admittance_controller/status", 10, subs_callback);
...
public:
  virtual void SetUp()
  {
    ...
    auto node = std::make_shared<rclcpp::Node>("test_cnr_admittance_controller");
...
protected:
  controller_interface::return_type SetUpController(const std::string & remove_name)
  {
    ...
    return AdmittanceControllerTest::SetUpController(
      "test_cnr_admittance_controller_no_overrides", parameter_overrides);
  }
...
protected:
  controller_interface::return_type SetUpController()
  {
    ...
    parameter_overrides.push_back(param);
    return AdmittanceControllerTest::SetUpController(
      "test_cnr_admittance_controller", parameter_overrides);
  }
};
